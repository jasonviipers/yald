 ...
 *
 * This module is behind the `yald_INTERACTIVE_PERMISSIONS_PTY` feature flag.
 *
 * Known limitations:
...
const LOG_FILE = join(homedir(), '.yald-debug.log')
...
function detectPermissionPrompt(lines: string[]): ParsedPermission | null {
  ...
  const toolMatch = joined.match(
    /(?:wants?\s+to\s+(?:use|run|execute)|Tool:\s*|tool_name:\s*)(\w+)/i
  )
  if (toolMatch) {
  ...
  const hasOptions =
    /(?:❯|›|>)\s*(?:Allow|Deny|Yes|No)/i.test(joined) ||
  ...
  const optionPatterns = [
    {
      pattern: /Allow\s+(?:for\s+(?:this\s+)?(?:project|session)|always)/i,
...
function parseToolCallLine(line: string): { toolName: string; input: string } | null {
  ...
  const match =
    line.match(/(?:⏳|⏳|✓|✗|⚡|🔧|Running|Executing)\s+(\w+)\s*(.*)/i) ||
...
export class PtyRunManager extends EventEmitter {
  ...
  startRun(requestId: string, options: RunOptions): PtyRunHandle {
    ...
    const args: string[] = ['--permission-mode', 'default']
   ...
   */
  private _checkPermissionInBuffer(
    requestId: string,
    handle: PtyRunHandle,
    currentLine: string
  ): void {
    ...
    log(
      `Permission prompt detected [${requestId}]: tool=${permission.toolName}, options=${permission.options.length}`
    )
    ...
    this.emit('normalized', requestId, {
      ...
      options: permission.options.map((o) => ({
        ...
        label: o.label,
        kind:
          o.label.toLowerCase().includes('deny') || o.label.toLowerCase().includes('reject')
            ? 'deny'
            : 'allow'
      }))
    } as NormalizedEvent)
  ...
  respondToPermission(requestId: string, _questionId: string, optionId: string): boolean {
    ...
    const isAllow =
      option.label.toLowerCase().includes('allow') || option.label.toLowerCase().includes('yes')
    const isDeny =
      option.label.toLowerCase().includes('deny') || option.label.toLowerCase().includes('reject')
