...
import { RemoteRunManager } from '../providers/remote-runner'
...
export class ControlPlane extends EventEmitter {
  ...
  private remoteRunManager: RemoteRunManager
  ...
  constructor(interactivePty = false) {
    ...
    this.remoteRunManager = new RemoteRunManager()
    ...
    this.hookServerReady = this.permissionServer
      .start()
      .then((port) => {
    ...
    this.permissionServer.on(
      ...
      (
        ...
        log(
          `Permission request [${questionId}]: tool=${toolRequest.tool_name} tab=${tabId.substring(0, 8)}… mode=${this.permissionMode}`
        )
    ...
    this.runManager.on(
      'exit',
      (requestId: string, code: number | null, signal: string | null, sessionId: string | null) => {
  ...
  private _wirePtyEvents(): void {
    ...
    // Exit events
    this.ptyRunManager.on(
      'exit',
      (requestId: string, code: number | null, signal: number | null, sessionId: string | null) => {
        // Clean up per-run token
        const runToken = this.runTokens.get(requestId)
        if (runToken) {
          this.permissionServer.unregisterRun(runToken)
  ...
  private _wireRemoteEvents(): void {
    this.remoteRunManager.on('normalized', (requestId: string, event: NormalizedEvent) => {
      const tabId = this._findTabByRequest(requestId)
      if (!tabId) return
    ...
    this.remoteRunManager.on(
      ...
      (requestId: string, code: number | null, signal: string | null, sessionId: string | null) => {
        ...
        if (code === 0) {
          ...
          const enriched = this.remoteRunManager.getEnrichedError(requestId, code)
          this.emit('error', tabId, enriched)
          this._setTabStatus(tabId, code === null ? 'dead' : 'failed')
        }
    ...
    this.remoteRunManager.on('error', (requestId: string, err: Error) => {
      const tabId = this._findTabByRequest(requestId)
      const inflight = this.inflightRequests.get(requestId)
      ...
      const enriched = this.remoteRunManager.getEnrichedError(requestId, null)
      enriched.message = err.message
      this.emit('error', tabId, enriched)
  ...
  async submitPrompt(tabId: string, requestId: string, options: RunOptions): Promise<void> {
    ...
    if (tab.activeRequestId) {
      ...
      log(
        `Tab ${tabId} busy — queuing request ${requestId} (queue depth: ${this.requestQueue.length + 1})`
      )
      return new Promise<void>((resolve, reject) => {
  ...
  private async _dispatch(tabId: string, requestId: string, options: RunOptions): Promise<void> {
    ...
    const useRemoteApi = options.provider?.transport === 'api'
    ...
    if (!useRemoteApi && this.permissionServer.getPort()) {
      const runToken = this.permissionServer.registerRun(
        tabId,
        requestId,
    ...
    try {
      if (useRemoteApi) {
        log(`Dispatching via remote provider transport: ${requestId}`)
        const handle = this.remoteRunManager.startRun(requestId, options)
        pid = handle.pid
      } else if (usePty) {
  ...
  cancel(requestId: string): boolean {
    ...
    if (this.remoteRunManager.isRunning(requestId)) {
      return this.remoteRunManager.cancel(requestId)
    }
  ...
  getHealth(): HealthReport {
    ...
    for (const [tabId, tab] of this.tabs) {
      ...
      if (tab.activeRequestId) {
        alive =
          this.runManager.isRunning(tab.activeRequestId) ||
          this.ptyRunManager.isRunning(tab.activeRequestId) ||
          this.remoteRunManager.isRunning(tab.activeRequestId)
      }
  ...
  getEnrichedError(requestId: string, exitCode: number | null): EnrichedError {
    ...
    if (this.remoteRunManager.isRunning(requestId)) {
      return this.remoteRunManager.getEnrichedError(requestId, exitCode)
    }
