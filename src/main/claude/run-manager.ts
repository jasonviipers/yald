...
import { resolveModelAttachments } from '../attachments'
import type { ClaudeEvent, RunOptions, EnrichedError } from '../../shared/types'

const MAX_RING_LINES = 100
const DEBUG = process.env.yald_DEBUG === '1'

// Appended to Claude's default system prompt so it knows it's running inside yald.
// Uses --append-system-prompt (additive) not --system-prompt (replacement).
const yald_SYSTEM_HINT = [
  'IMPORTANT: You are NOT running in a terminal. You are running inside yald,',
  'a desktop chat application with a rich UI that renders full markdown.',
  'yald is a GUI wrapper around coding and research models — the user sees your output in a',
  'styled conversation view, not a raw terminal.',
  '',
  'Because yald renders markdown natively, you MUST use rich formatting when it helps:',
  '- Always use clickable markdown links: [label](https://url) — they render as real buttons.',
  '- When the user asks for images, and public web images are appropriate, proactively find and render them in yald.',
  ...
  '- Do not ask whether yald can render images; assume it can.',
...
// safety. This is a deliberate yald policy choice, not native Claude parity.
...
const DEFAULT_ALLOWED_TOOLS = ['Bash', 'Edit', 'Write', 'MultiEdit', ...SAFE_TOOLS]
...
export class RunManager extends EventEmitter {
  ...
  private _findClaudeBinary(): string {
    ...
    try {
      return execSync('/bin/zsh -ilc "whence -p claude"', {
        encoding: 'utf-8',
        env: getCliEnv()
      }).trim()
    } catch {}
    ...
    try {
      return execSync('/bin/bash -lc "which claude"', {
        encoding: 'utf-8',
        env: getCliEnv()
      }).trim()
    } catch {}
  ...
  startRun(requestId: string, options: RunOptions): RunHandle {
    ...
    if (options.hookSettingsPath) {
      // yald-scoped hook settings: the PreToolUse HTTP hook handles permissions
    ...
    // Always tell Claude it's inside yald (additive, doesn't replace base prompt)
    args.push('--append-system-prompt', yald_SYSTEM_HINT)
    ...
    parser.on('event', (raw: ClaudeEvent) => {
      ...
      if (
        ...
        (raw.type === 'system' && 'subtype' in raw && (raw as any).subtype === 'permission_request')
      ) {
      ...
      if (raw.type === 'result') {
        log(
          `Run complete [${requestId}]: sawPermissionRequest=${handle.sawPermissionRequest}, denials=${handle.permissionDenials.length}`
        )
        try {
          child.stdin?.end()
        } catch {}
    ...
    void (async () => {
      ...
      this._ringPush(handle.stderrTail, `[attachments] ${message}`)
      log(`Attachment preparation failed [${requestId}]: ${message}`)
      ...
      child.stdin!.write(fallbackMessage + '\n')
    })
