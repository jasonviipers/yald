 ...
 *
 * This is a yald-owned permission broker that approximates Claude Code's
 ...
 *   - Per-run settings files (only yald-spawned sessions see the hook)
 */
...
function isSafeBashCommand(command: unknown): boolean {
  ...
  for (const segment of segments) {
    ...
    if (
      segment.includes('>') &&
      !segment.includes('>/dev/null') &&
      !segment.includes('2>/dev/null') &&
      !segment.includes('2>&1')
    )
      return false
  }
...
export class PermissionServer extends EventEmitter {
  ...
  generateSettingsFile(runToken: string): string {
    ...
    const dir = join(tmpdir(), 'yald-hook-config')
    try {
      mkdirSync(dir, { recursive: true, mode: 0o700 })
    } catch {}

    const filePath = join(dir, `yald-hook-${runToken}.json`)
  ...
  private async _handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (DEBUG) {
      log(
        `Hook request: tool=${toolRequest.tool_name} id=${toolRequest.tool_use_id} session=${toolRequest.session_id} tab=${registration.tabId.substring(0, 8)}…`
      )
    } else {
    ...
    if (toolName === 'Bash' && isSafeBashCommand(toolRequest.tool_input?.command)) {
      if (DEBUG)
        log(`Auto-allowing safe Bash: ${String(toolRequest.tool_input?.command).substring(0, 80)}`)
      res.writeHead(200, { 'Content-Type': 'application/json' })
